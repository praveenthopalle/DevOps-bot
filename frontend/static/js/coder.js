...
function language() {
    ...
    errorCallback = function(data) {
        console.log(data)
        $("#my-login").modal('show')
        myAlertPure("Error 错误", "The back-end service interface cannot be accessed. Please check the terminal service log and browser console. (Usually the back-end service is not started, Or exists <a href='https://github.com/PKTAI/DevOpsGPT/blob/master/docs/DOCUMENT.md#configuration-details' target='_blank'> Cross-domain problem? </a>)<br /><br />无法访问后端服务接口，请检查终端服务日志以及浏览器控制台报错信息。（通常是后端服务没有启动，或存在 <a href='https://github.com/PKTAI/DevOpsGPT/blob/master/docs/DOCUMENT_CN.md#%E5%9F%BA%E7%A1%80%E9%85%8D%E7%BD%AE%E7%B1%BB' target='_blank'>跨域问题？</a>）")
    }
